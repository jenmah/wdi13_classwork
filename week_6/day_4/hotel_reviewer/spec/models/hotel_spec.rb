...
describe Hotel do
  ...
  it 'must have a country' do
    hotel = Hotel.new(name: 'The Ice Hotel', city: 'London')
    expect(hotel.valid?).to eq false
  end

  context 'no ratings' do
    it 'should return a message saying no reviews' do
      ice_hotel = Hotel.new(name: 'The Ice Hotel', city: 'London', country: 'England')
  ...
  context 'multiple ratings' do
    it 'should return the average of all the ratings' do
      ice_hotel.reviews << Review.new(description: 'great!', rating: 5)
      ice_hotel.reviews << Review.new(description: 'decent', rating: 4)
